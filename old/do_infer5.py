...
def seg_by_water(source_np, ref_np, is_mirror_pulp = False, count_bg = True):
...
def init_predictors(cuda = 0):
    ...
    predictor916.initialize_from_trained_model_folder(
        join(nnUNet_results, "Dataset916_MICCAILabeled/nnUNetTrainer__nnUNetPlans__3d_new"),
        use_folds = (0,),
    ...
    # predictor821.initialize_from_trained_model_folder(
    #     join(nnUNet_results, "Dataset821_MICCAILabeled/nnUNetTrainer_DASegOrd0_NoMirroring__nnUNetPlans__2d_new"),
    #     use_folds = (0,),
    #     checkpoint_name = "checkpoint_best2.pth",
    # )
...
def main(input_dir, output_dir, verbose = False):
    ...
    for file in input_dir.glob("*.nii.gz"):
        ...
        out_np = seg_by_water(ret916_np, ret821_np, is_mirror_pulp = not is_reverse)
