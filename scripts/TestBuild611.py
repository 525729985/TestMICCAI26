...
def mapping_DS627() -> Dict[int, int]:
    mapping = {}
    mapping.update({i: 1 for i in range(1, 9)})
    mapping.update({i: 2 for i in range(9, 17)})
    mapping.update({i: 3 for i in range(17, 25)})
    mapping.update({i: 4 for i in range(25, 33)})

    mapping.update({i: 1 for i in range(33, 41)})
    mapping.update({i: 2 for i in range(41, 49)})
    mapping.update({i: 3 for i in range(49, 57)})
    mapping.update({i: 4 for i in range(57, 65)})
    return mapping
...
def process_labels(
    files: str, lbl_dir_in: str, lbl_dir_out: str, mapping: Dict[int, int] | None, n_processes: int = 12
) -> None:
...
def process_ds(
    root: str, out_root: str, input_ds: str, output_ds: str, mapping: dict | None, image_link: str = None
) -> None:
    ...
    if mapping is not None:
        ...
        for k, v in label_dict.items():
            if v in mapping.keys() and v not in label_dict_new.values():
    ...
    if os.path.isfile(split_input_path):
        print(f"copy splits_final.json from {input_ds} to {output_ds}.")
